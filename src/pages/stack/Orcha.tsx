...
import { ExecutionVisual } from '@/components/system/StackVisuals';
        ...
        <div className="max-container max-w-3xl">
          <Reveal>
            ...
            <p className="text-lg md:text-xl text-muted-foreground leading-relaxed mb-6">
              ORCHA is the orchestration engine at the centre of the LocalHouseLLM stack. It is the layer that
              decides what should happen, in what order, by which expert, and how to put the pieces back together.
            </p>
            <p className="text-base text-muted-foreground leading-relaxed mb-12">
              A modern AI request is rarely one thing. It is a question, a retrieval, a calculation, a synthesis,
              and a verification — often interleaved. ORCHA makes those steps explicit so they can be inspected,
              retried, and improved.
            </p>
          </Reveal>

          <Reveal className="mb-14">
            <h2 className="font-display text-2xl md:text-3xl font-semibold mb-8">The orchestration loop</h2>
            <ExecutionVisual />
          </Reveal>
          ...
          <Reveal>
            <h2 className="font-display text-2xl md:text-3xl font-semibold mb-5">Why orchestration is not communication</h2>
            <p className="text-muted-foreground mb-4">
              It is tempting to treat &ldquo;the orchestrator&rdquo; and &ldquo;the protocol&rdquo; as one thing.
              They are not. <Link to="/stack/aicl" className="text-foreground underline underline-offset-4">AICL</Link>{' '}
              defines how messages move. ORCHA decides which messages to send, and what to do with the responses.
              Keeping these separate is what makes either of them upgradable in isolation.
            </p>
            <p className="text-muted-foreground mb-12">
              It is also what allows third-party orchestrators. Anyone can ship their own routing policy on top of
              AICL without forking the rest of the stack.
            </p>
          </Reveal>

          <Reveal>
            <h2 className="font-display text-2xl md:text-3xl font-semibold mb-5">A 10-minute integration sketch</h2>
            <div className="p-6 rounded-sm border border-border font-mono text-xs md:text-sm leading-relaxed bg-card mb-12 whitespace-pre overflow-x-auto">{`import { Orcha } from "@localhousellm/orcha";
...
const result = await orcha.handle({
  ...
  payload: { question: "..." },
            ...
            <p className="text-muted-foreground mb-12">
              ...
              <Link to="/docs" className="text-foreground underline underline-offset-4">docs</Link>. Numbers for
              routing quality, latency, and confidence calibration are tracked on the{' '}
              <Link to="/benchmarks" className="text-foreground underline underline-offset-4">benchmarks page</Link>.
            </p>
            ...
            <div className="flex flex-wrap gap-3">
              <a href="https://github.com/LocalHouseLLM/orcha01" target="_blank" rel="noopener noreferrer">
                <Button className="rounded-sm bg-foreground text-background hover:bg-primary hover:text-primary-foreground h-11 px-6">
                  ORCHA on GitHub <ArrowRight className="w-4 h-4 ml-1" />
                </Button>
              </a>
              <Link to="/stack">
